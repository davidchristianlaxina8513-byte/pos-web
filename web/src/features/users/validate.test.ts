...
test('email, password, and role are enforced', () => {
  expect(
    validateNewStaff({ username: 'not-an-email', password: 'secret1', role: 'cashier' }),
  ).toEqual({ ok: false, error: 'Enter a valid email address.' });
  expect(
    validateNewStaff({ username: 'a@elvira.cafe', password: 'short', role: 'cashier' }),
  ).toEqual({ ok: false, error: 'Password must be at least 6 characters.' });
  expect(
    validateNewStaff({ username: 'a@elvira.cafe', password: 'secret1', role: 'owner' }),
